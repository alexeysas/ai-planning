...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            if proposed_node.prenodes <= self.s_levels[level]:
                ...
                # 2. connect the nodes to the previous S literal leve
                for s_node in self.s_levels[level]:
                    if s_node in proposed_node.prenodes:
    ...
    def add_literal_level(self, level):
        ...
        for action_node in self.a_levels[level - 1]:
            ...
            #connect
            for s_node in self.s_levels[level]:
                if s_node in action_node.effnodes:
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for s_node in node_a1.parents:
            for s_node2 in node_a2.parents:
                if s_node.is_mutex(s_node2):
                    return True
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for action in node_s1.parents:
            for action2 in node_s2.parents:
                if not action.is_mutex(action2):
                    return False
    ...
    def h_levelsum(self) -> int:
        ...
        """
        level_sum = 0
        goal_fluents = set(self.problem.goal)

        for i, level in enumerate(self.s_levels):
            for node in level:
                if node.symbol in goal_fluents and node.is_pos:
                    level_sum += i
                    goal_fluents.remove(node.symbol)
                    if not goal_fluents:
                        break
